...
class View(object):
    ...
    def get_main(self):
        ...
        self.MainView.pushButtonSettings.clicked.connect(
            lambda: self.MainView.stackedWidget.setCurrentWidget(self.MainView.settingsWidget))
        ...
        self.MainView.pushButtonExecuteSelectedFile.clicked.connect(self.Controller.execute_selected_file)
        ...
        self.MainView.treeViewSystem.setModel(self.Controller.Model.FileSystemModel)
        ...
        self.MainView.treeViewRequestData.horizontalScrollBar().setStyleSheet(scrollbar_stylesheet)
        self.MainView.treeViewRequestData.verticalScrollBar().setStyleSheet(scrollbar_stylesheet)
        ...
        self.MainView.listViewFile.setEditTriggers(QtWidgets.QAbstractItemView.NoEditTriggers)
        self.MainView.listViewFile.doubleClicked.connect(lambda : self.MainView.lineEditSelectedFile.setText(self.MainView.listViewFile.currentIndex().data(QtCore.Qt.DisplayRole)))

        self.MainView.listViewFileHistory.horizontalScrollBar().setStyleSheet(scrollbar_stylesheet)
        self.MainView.listViewFileHistory.verticalScrollBar().setStyleSheet(scrollbar_stylesheet)
        self.MainView.listViewFileHistory.setEditTriggers(QtWidgets.QAbstractItemView.NoEditTriggers)
        
        
        self.MainView.lineEditSelectedFile.setAlignment(QtCore.Qt.AlignCenter)
        ...
        self.MainView.lineEditSearcher.returnPressed.connect(lambda : self.search(self.MainView.lineEditSearcher.text()))
        self.MainView.pushButtonSearch.clicked.connect(lambda : self.search(self.MainView.lineEditSearcher.text()))
        
        self.MainView.lineEditSearcher.setPlaceholderText('Ingresa el nombre de la llave')
        ...
        try:
            ...
            for row in range(row_count):
                ...
                if text in str(model.index(row, 0, QtCore.QModelIndex()).data(QtCore.Qt.DisplayRole)):
                    self.MainView.treeViewRequestData.expand(model.index(row, 0, QtCore.QModelIndex()))

                            
                    
                else:
                    self.MainView.treeViewRequestData.collapse(model.index(row, 0, QtCore.QModelIndex()))
                    #self.MainView.treeViewRequestData.collapse(model.index(row, 0, QtCore.QModelIndex()))
